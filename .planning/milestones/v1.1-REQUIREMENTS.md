# Requirements Archive: v1.1 Multi-User + Capacity + Branded UI
...
Checkbox legend: `[x]` = shipped + verified at code level; `[~]` = shipped at code level, manual verification deferred to v1.2 (see `FUTURE_DIRECTIONS.md` §8).
...
- [x] **AUTH-05**: Public `/signup` form accepts email + password and creates a Supabase `auth.users` row (email/password only — no OAuth in v1.1)
- [x] **AUTH-06**: Signup form rejects duplicate emails generically without confirming registration ("If your email is registered, you'll receive a verification email") to prevent email-enumeration
- [x] **AUTH-07**: New users must verify their email before they can log in (hard gate; orphan accounts cleaned up via `accounts.deleted_at`)
- [x] **AUTH-08**: `/auth/confirm` Route Handler exchanges Supabase token-hash codes for sessions (verifyOtp pattern) for signup, password recovery, and future magic-link/email-change flows
- [x] **AUTH-09**: User can request a password-reset email at `/forgot-password`; landing page is `/auth/reset-password` and POSTs the new password through `/auth/confirm` (recovery type)
- [x] **AUTH-10**: Existing single-tenant owner user (`ajwegner3@gmail.com`) continues to log in after email-confirmation is re-enabled (pre-flight UPDATE on `email_confirmed_at` if null)
- [x] **AUTH-11**: `/api/auth/*` endpoints (signup, login, password-reset) are rate-limited per IP via the existing `rate_limit_events` table

### Account Provisioning + Onboarding (Phase 10)

- [x] **ONBOARD-01**: First-login wizard runs at `/onboarding` for accounts where `onboarding_complete = false`; wizard is 3 steps (display name + slug → timezone confirm → first event type or skip)
- [x] **ONBOARD-02**: Wizard creates an `accounts` row atomically with `slug`, `display_name`, `timezone`, `owner_user_id`, `owner_email`, and `onboarding_complete = true` on final-step submission
- [x] **ONBOARD-03**: Wizard seeds 5 default `availability_rules` rows (Mon–Fri, 09:00–17:00) in the user's local timezone (captured at signup via `Intl.DateTimeFormat().resolvedOptions().timeZone`)
- [x] **ONBOARD-04**: Wizard creates 1 default `event_types` row ("30 Minute Meeting", `slug=30min`, `duration_minutes=30`, `max_bookings_per_slot=1`, active) on completion
- [x] **ONBOARD-05**: Slug picker validates `/^[a-z0-9-]{3,40}$/` and rejects entries in the consolidated `RESERVED_SLUGS` set (single source of truth at `lib/reserved-slugs.ts` — replaces v1.0's 2-file duplication)
- [x] **ONBOARD-06**: Slug picker provides real-time collision detection via `/api/check-slug` (debounced)
- [x] **ONBOARD-07**: When a slug is taken, the picker suggests 3 alternatives (e.g., `andrew-2`, `andrew-nsi`, `andrewbookings`) — appends numeric suffix + email-prefix variants
- [x] **ONBOARD-08**: User receives a welcome email post-signup with their booking link (separate from the Supabase verification email)
- [x] **ONBOARD-09**: Dashboard shows an onboarding progress checklist for accounts in their first 7 days (Set availability ✓ / Customize event ✓ / Share your link ☐); user can dismiss; dismissal persists in `accounts.onboarding_checklist_dismissed_at`

### Account Settings + Profile (Phase 10)

- [x] **ACCT-01**: New `/app/settings/profile` route lets the user view email (read-only), change password (current-password challenge), edit display name, and view account slug
- [x] **ACCT-02**: User can soft-delete their own account from `/app/settings/profile` (sets `accounts.deleted_at`; account hidden from public; auth.users row preserved for v1.1; hard delete deferred to v1.2 cron purge)
- [x] **ACCT-03**: Soft-deleted accounts return 404 on all public surfaces (`/[account]/*`, `/embed/[account]/*`)
...
- [x] **BRAND-05**: New `accounts.background_color TEXT` column (nullable hex; null falls back to `gray-50`)
- [x] **BRAND-06**: New `accounts.background_shade TEXT CHECK (background_shade IN ('none','subtle','bold')) DEFAULT 'subtle'` column controls gradient intensity
- [x] **BRAND-07**: Branding editor at `/app/branding` adds `background_color` swatch picker and `background_shade` 3-button toggle with live preview iframe

### Owner UI (Phase 12)

- [x] **UI-01**: Dashboard global typography uses Inter via `next/font/google` with `tracking-tight`
- [x] **UI-02**: Dashboard surfaces use `bg-gray-50` page background + `gray-900` primary text per Cruip "Simple Light"
- [x] **UI-03**: Dashboard renders a floating glass header pill (`fixed top-2 md:top-6`)
- [x] **UI-04**: Surfaces with `background_shade != 'none'` render gradient blur-circle decorative backgrounds derived from the account's `brand_primary` and `background_color`; hidden when `background_shade = 'none'`
- [x] **UI-05**: Sidebar IA: Home / Event Types / Availability / Bookings / Branding / Settings (flat list; Settings expands to Reminders + Profile in-place)
- [x] **UI-06**: New `/app/home` (or `/app`) Home tab renders a monthly calendar (`react-day-picker@9`) with `modifiers` highlighting days that have bookings
- [x] **UI-07**: Clicking a day in the Home calendar opens a shadcn `Sheet` drawer listing that day's bookings
- [x] **UI-08**: Day-detail drawer rows include a dropdown menu with View, Cancel, and Copy-link actions (reusing Phase 8 booking-detail-page primitives)
- [x] **UI-09**: Embed snippet dialog (`EmbedCodeDialog`) widens to `sm:max-w-2xl` (or wider) so the snippet does not overflow / overlap

### Public Surfaces (Phase 12)

- [x] **UI-10**: Public booking page `/[account]/[event-slug]` is restyled per Cruip "Simple Light": section rhythm `py-12 md:py-20`, `max-w-3xl` slot picker, gradient backgrounds keyed off the account's branding tokens
- [x] **UI-11**: Embed widget `/embed/[account]/[event-slug]` adopts the same restyle, scoped chromeless (no nav, no header pill); height-postMessage protocol continues to work
- [x] **UI-12**: `/login`, `/signup`, `/auth/reset-password` adopt the skill's split-panel auth-page pattern (form left, decorative branded panel right on `lg:`)
- [x] **UI-13**: `/[account]` public index page is restyled as a polished personal landing card (logo + display name + brand color + active event types) — the URL contractors share on business cards

### Email Branding (Phase 12)

- [x] **EMAIL-09**: Booker confirmation, reminder, cancel, and reschedule emails (and owner notification + cancel + reschedule) ship a per-account branded header (gradient with solid-color fallback for Outlook + Yahoo, OR solid-color-only — planner picks during Phase 12 plan)
- [x] **EMAIL-10**: Booker confirmation email includes a plain-text alternative (`text: stripHtml(html)`) — mirrors reminder pattern; closes FUTURE_DIRECTIONS.md §3 backlog
- [x] **EMAIL-11**: Email footer includes the NSI mark image (sets `NSI_MARK_URL`) — closes FUTURE_DIRECTIONS.md §3 backlog
- [x] **EMAIL-12**: Per-template branding visual smoke verified across all 6 transactional surfaces (booker × owner × confirm/cancel/reschedule) — closes FUTURE_DIRECTIONS.md §3 6-row smoke backlog
...
- [x] **QA-14**: Andrew explicit ship sign-off: "ship v1.1"
- [x] **QA-15**: `FUTURE_DIRECTIONS.md` updated with v1.1 carry-overs to v1.2 (anything deferred during marathon QA)
...
**Shipped:** 62 of 67 v1.1 requirements at code level (62 marked `[x]` or `Complete`).
**Deferred:** 5 of 67 v1.1 requirements to v1.2 (QA-09..QA-13 marathon items, marked `[~]` / `DEFERRED-V1.2`) — Andrew marathon waiver 2026-04-30 verbatim "consider everything good. close out the milestone."
**Adjusted scope mid-milestone:**
- Phase 12 plan count revised from 6 to 7 (Plan 12-04 split into 12-04a + 12-04b mid-execution to scope home-tab Sheet drawer separately).
- Phase 12.5 inserted between Phase 12 and Phase 13 as scope extension (per-account chrome theming + email token unification + FloatingHeaderPill removal). Then Phase 12.6 inserted same day as a course-correction by Andrew after Vercel review showed 12.5's color-mix tinting at 6-14% was visually-indistinguishable; 12.6 replaced with direct hex application.
- Phase 12.5 reqs (BRAND-08/09, UI-14/15/16/17, EMAIL-13) shipped at code level but DEPRECATED in code by Phase 12.6 (DB columns kept for one v1.1 release window before v1.2 DROP). Marked Complete since the requirements were satisfied; DEPRECATED note in description.
- EMAIL-10 scope extended beyond minimum to all booker-facing emails (cancel + reschedule), not just confirmation.

**Dropped:** None.

**v1.0 marathon QA carry-overs (EMBED-07, EMAIL-08, QA-01..06)** RE-DEFERRED to v1.2 at v1.1 kickoff per Andrew scope-cut 2026-04-27 and RE-confirmed at v1.1 close 2026-04-30.

---

*Requirements defined: 2026-04-27 for v1.1 milestone*
*Archived: 2026-04-30 as part of v1.1 milestone completion*
