...
**v1.6 — shipped 2026-05-06 (25 of 25 requirements complete; 100%):**

- ✓ AVAIL-01: Owner opens date override editor and sees "Add unavailable windows" mode; prior "Enter available times" mode removed — v1.6 (Phase 32-02; override-modal.tsx full rewrite)
- ✓ AVAIL-02: Owner can add, edit, and remove multiple unavailable time windows on a single date; non-overlapping validation enforced at schema layer — v1.6 (Phase 32-02)
- ✓ AVAIL-03: "Block entire day" toggle hides+preserves unavailable-windows list; toggling off restores prior window state — v1.6 (Phase 32-02; hide+preserve pattern, not wipe)
- ✓ AVAIL-04: `lib/slots.ts` `windowsForDate()` computes slots as weekly-hours MINUS unavailable windows via `subtractWindows()` helper; buffer + capacity logic unchanged — v1.6 (Phase 32-01; MINUS semantics with 3 legacy rows wiped)
- ✓ AVAIL-05: Owner sees inline affected-bookings preview (booker name + account-TZ times + event-type label) before commit when windows overlap existing bookings — v1.6 (Phase 32-02; `previewAffectedBookingsAction`)
- ✓ AVAIL-06: On confirmation, existing confirmed bookings inside unavailable windows auto-cancelled via existing cancel lifecycle — v1.6 (Phase 32-03; `commitInverseOverrideAction` with `skipOwnerEmail=true`)
- ✓ AVAIL-07: Booker cancel email includes rebook CTA; passes quota guard (EMAIL-23 pre-flight gates commit if batch would exceed quota) — v1.6 (Phase 32-03; quota math = N emails for N bookings with skipOwnerEmail)
- ✓ AVAIL-08: v1.5 invariants preserved — buffer-after-minutes applied; cross-event EXCLUDE GIST constraint binding; partial-unique capacity index binding; race-safety unchanged — v1.6 (Phase 32-01; Andrew live-verified 8/8 scenarios)
- ✓ PUSH-01: Owner can open "Pushback" action from `/app/bookings` page — v1.6 (Phase 33-01; `PushbackDialogProvider` + per-day shortcut buttons)
- ✓ PUSH-02: Pushback dialog defaults to today's date in account timezone; date picker to change; confirmed bookings listed chronologically — v1.6 (Phase 33-01)
- ✓ PUSH-03: Owner picks an anchor booking; bookings before anchor unaffected — v1.6 (Phase 33-01; anchor radio selection)
- ✓ PUSH-04: Owner enters delay in minutes or hours (positive integer); dialog computes new times in real time — v1.6 (Phase 33-02; `computeCascadePreview()` pure module)
- ✓ PUSH-05: Optional reason text field (max ~280 characters); same reason included in every pushback email — v1.6 (Phase 33-03/04; reason threaded to `rescheduleBooking` → `sendRescheduleEmails` → template)
- ✓ PUSH-06: Smart cascade — each booking moves only if prior booking's new end > its original start; otherwise gap absorbed and booking left in place — v1.6 (Phase 33-02; `computeCascadePreview()` with `snapToNextSlotMs` per booking's own `duration_minutes`)
- ✓ PUSH-07: Bookings whose new start pushes past end-of-workday still processed through same email lifecycle; flagged PAST_EOD in preview — v1.6 (Phase 33-02; 1440 sentinel for no-rules days via `getEndOfDayMinute`)
- ✓ PUSH-08: Pre-commit preview shows moved / absorbed / past-EOD bookings plus total email count + remaining quota — v1.6 (Phase 33-02; `previewPushbackAction` + EMAIL-22 quota gate)
- ✓ PUSH-09: On confirm, every moved booking rides existing reschedule lifecycle — `booking_events` audit row, .ics METHOD:REQUEST SEQUENCE+1, calendar invite email, cancel link — v1.6 (Phase 33-03; `commitPushbackAction` via `rescheduleBooking`)
- ✓ PUSH-10: Pushback emails include "sorry for the inconvenience" copy + reason text (when actor='owner' and reason non-empty); LD-07 booker-neutrality preserved — v1.6 (closed by orchestrator commit `2aa9177` after Phase 33 verifier; apology + "Reason:" callout block in booker template)
- ✓ PUSH-11: Pushback is race-safe — concurrent cancel/reschedule during commit does not produce duplicate emails; v1.4 EXCLUDE GIST + v1.1 capacity index bind on new times; ABORT-on-diverge if ID set differs between preview and commit — v1.6 (Phase 33-03; `commitPushbackAction` abort-on-diverge strategy)
- ✓ PUSH-12: Post-commit summary shows per-row status (Sent/Failed/Conflict/Stale/Skipped) + retry button for `email_failed` rows — v1.6 (Phase 33-04; `retryPushbackEmailAction` with fresh-token mint + quota guard + per-row `useTransition`)
- ✓ EMAIL-21: Quota guard `checkAndConsumeQuota()` refuses to send when day count is at 200 for ALL 7 email paths; v1.1 carve-out removed — v1.6 (Phase 31-02; 7 sender modules all guarded)
- ✓ EMAIL-22: Pushback batch pre-flight surfaces email count + remaining quota; Confirm disabled if batch would exceed cap — v1.6 (Phase 33-02; `previewPushbackAction` EMAIL-22 gate; Phase 31 visual vocabulary reused verbatim)
- ✓ EMAIL-23: Date-override auto-cancel batch pre-flight surfaces conflict and disables commit if would exceed quota — v1.6 (Phase 32-03; `commitInverseOverrideAction` HARD quota pre-flight)
- ✓ EMAIL-24: A refused single send returns clear error to the user-facing path; no silent failure — v1.6 (Phase 31-02/03; `emailFailed: "quota"` signal; inline AlertDialog error for manual reminder; differentiated cancel toast)
- ✓ EMAIL-25: All quota refusals logged with PII-free structured fields (`code`, `account_id`, `sender_type`, `count`, `cap`); no booker_email/booker_name/booker_phone/ip in log — v1.6 (Phase 31-01; `logQuotaRefusal()` helper; 21 refuse-send tests with negative PII assertions)
...
**Production state at v1.6 ship (2026-05-06):**

- ~22,800 LOC TypeScript/TSX in the runtime tree (v1.0: 20,417; v1.1: 29,450; v1.2: 21,871; v1.3: 22,071; v1.4: ~22,071; v1.5: 22,356; v1.6: ~22,800 estimated — Phase 31 added email-guard wiring across 12 files; Phase 32 added override modal rewrite + batch-cancel server action; Phase 33 added pushback dialog + 4 server actions; runtime LOC smaller than total diff due to `.planning/` artifact bulk).
- v1.6 delta: +15,365 / -340 across 78 files (includes `.planning/` planning artifacts; runtime code diff is much smaller).
- ~563 cumulative commits (222 v1.0 + 135 v1.1 + 91 v1.2 + 34 v1.3 + 50 v1.4 + 31 v1.5 + 53 v1.6; v1.6 git range `2345095` → `ca02c90`).
- 30 of 31 test files pass (~285 tests) at v1.6 ship; vitest `resolve.alias` regex fix (Plan 32-03) raised green count from 24/31 to 30/31. Lone failing file (`bookings-api.test.ts`) is pre-existing fixture mismatch not introduced by v1.6.
...
- `accounts` table schema (post-v1.6): unchanged from v1.5 — `id`, `slug`, `name`, `owner_email`, `logo_url`, `brand_primary`, `timezone`, `onboarding_complete`. `buffer_minutes` PERMANENTLY DROPPED in v1.5.
- `event_types` table key columns: `buffer_after_minutes` (per-event-type post-buffer; default 0; range 0-360 step 5). `buffer_before_minutes` exists at default 0 (BUFFER-07 wiring deferred to v1.7+).
- `bookings` table new column (v1.6): `confirmation_email_sent boolean NOT NULL DEFAULT true` with partial index `bookings_confirmation_email_unsent_idx ON (account_id) WHERE confirmation_email_sent = false` — used by `/app/bookings` unsent-confirmations banner.
- `bookings` table invariants (v1.4 sealed, v1.5 unchanged, v1.6 unchanged): `bookings_capacity_slot_idx` partial unique on `(event_type_id, start_at, slot_index) WHERE status='confirmed'` (v1.1 group-booking capacity); `bookings_no_account_cross_event_overlap` EXCLUDE GIST on `(account_id, event_type_id WITH <>, during)` WHERE confirmed (v1.4 cross-event-type guard); generated `during tstzrange` column with `[)` half-open bound (v1.4).
- `email_send_log` table: `category` CHECK constraint now accepts 12 values (5 legacy signup-side + 7 booking-side added in v1.6). Canonical quota guard `lib/email-sender/quota-guard.ts` exports `checkAndConsumeQuota`, `getRemainingDailyQuota`, `logQuotaRefusal`, `QuotaExceededError`.
- `date_overrides` table: 3 legacy `is_closed=false` custom-hours rows wiped in v1.6 migration `20260505120000`. All overrides now follow Phase 32 MINUS semantics — `is_closed=true` = full-day block, `is_closed=false` = unavailable windows (subtracted from weekly base via `subtractWindows()`).

**Production state historical (preserved for reference):**

- v1.5 ship (2026-05-05): 22,356 LOC TypeScript/TSX; +7,502 / -990 across 55 files; ~510 cumulative commits; 228 passing + 9 skipped automated tests; v1.5 git range `ca402a7` → `6dc91e8`.
...
| v1.6 Refuse-send fail-closed across all 7 senders (not save-and-flag for booking confirmations only) | The v1.1 carve-out exempted bookings/reminders to protect core flow. Pushback's higher email volume (5+ per batch) made the quota a real ceiling that needed system-wide enforcement. Phase 31 closed the carve-out by wiring all 7 sender modules through `checkAndConsumeQuota()`. Booking confirmations refused at cap use save-and-flag (`confirmation_email_sent=false`); owner-triggered paths receive synchronous `emailFailed: "quota"` | ✓ Good — Phase 31 complete; 21 refuse-send tests + PII-free log shape verified. Pattern: fail-closed is the default for all new email paths going forward. |
| v1.6 Wipe migration over dual-read CP-03 for legacy date_overrides rows (Plan 32-01) | Production diagnostic confirmed exactly 3 legacy `is_closed=false` custom-hours rows. Adding a `semantics_v2` column (CP-03 dual-read protocol) for 3 rows is unnecessary thrash. Clean wipe via DML DELETE is simpler and safer | ✓ Good — Phase 32-01 migration applied; 0 rows remain; slot engine MINUS semantics now operates on a homogeneous dataset. Reusable pattern: when row count is ≤ 5 and all rows can be wiped without business impact, DML wipe is cleaner than schema column migration. |
| v1.6 ABORT-on-diverge race strategy for pushback commit (NOT union as in Phase 32) | Phase 32's `commitInverseOverrideAction` unions the preview-approved IDs with post-write re-query IDs — no booking is missed. Phase 33 ABORTS instead — cascade math is order-dependent; any addition or removal on the day invalidates all computed new times. A partial commit with stale cascade math would move bookings to wrong times | ✓ Good — Phase 33-03; `commitPushbackAction` returns `{ ok: false, diverged: true, message }` on any ID set difference. Dialog resets to editing state with refreshed bookings list. Andrew live-verified abort scenario (scenario 3 of 8). |
| v1.6 Per-booking slot step uses each booking's own `duration_minutes` (OQ-2) | A shared account-wide step would snap 30-min bookings and 60-min bookings to the same boundary, potentially producing invalid new times for shorter bookings (e.g. a 30-min booking snapped to a 60-min boundary). Using each booking's own duration gives correct boundary alignment | ✓ Good — Phase 33-02; `snapToNextSlotMs` called per booking with that booking's `duration_minutes`. Pattern: when rounding/snapping to a grid, the grid step must come from the item being snapped, not a global setting. |
| v1.6 1440 sentinel for no-rules days in cascade (OQ-5) | `getEndOfDayMinute` returns 1440 (24 hours in minutes) when `windowsForDate` returns null or empty. `isPastEod` short-circuits immediately — bookings on days with no availability rules never get PAST_EOD badge (they can't be past an end-of-day that doesn't exist) | ✓ Good — Phase 33-02; logical safe-default that prevents false PAST_EOD flags on owner-set holiday blocks or days with date_overrides that fully block the day. |
| v1.6 PUSH-10 reason callout pattern (apology + "Reason:" block when actor='owner', LD-07 booker-neutrality, mirrors cancel email) | Phase 33-03 initially deferred reason text as tech debt (template had no reason block; RescheduleBookingArgs had no reason field). Phase 33 verifier returned `human_needed` for PUSH-10. Orchestrator closed the gap in commit `2aa9177` by threading reason through `rescheduleBooking` → `sendRescheduleEmails` → template. LD-07 booker-neutrality preserved — no NSI branding or owner identity revealed in booker-facing copy | ✓ Good — verified by Andrew in scenario 7 of 8 during Phase 33 verifier pass. Pattern: reason text appears iff `actor === 'owner' && reason` is non-empty; template renders apology line + "Reason: [text]" callout block. Mirrors cancel email. |
| v1.6 `skipOwnerEmail`/`actor` as canonical owner-batch email suppression pattern (Phases 32 + 33) | Owner-initiated batches (auto-cancel + pushback) would send N duplicate owner notifications without suppression. One per-booking owner notification is correct for individual actions, but N notifications for an N-booking batch is spam. `skipOwnerEmail=true` + `actor="owner"` established across both `cancelBooking` and `rescheduleBooking` | ✓ Good — symmetric implementation across Phase 32 cancel path and Phase 33 reschedule path. Reusable for any future owner-batch operation. Quota math = N (not 2N) when `skipOwnerEmail=true`. |
| v1.6 Critical lesson: plans must reference real DB columns — grep migrations before naming fields | Phase 33-02 invented non-existent column `booker_first_name` in plan body. Real column is `booker_name` (full name, established in v1.0). The invented name caused a silent runtime failure (empty anchor list) caught only during live test. Fixed by orchestrator in commit `bba0e18` | ⚠ Revisit — new planner discipline required: before naming any column in a plan body, grep `supabase/migrations/` for the actual column definition. The `firstNameOf(fullName)` render helper (split on whitespace at render time) is the canonical first-name derivation pattern going forward. |
