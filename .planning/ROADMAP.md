...
- [x] 46-01-migration-sync-repair-PLAN.md — Phase 41 (`20260510120000_phase41_stripe_billing_foundation`) registered in `schema_migrations` via `supabase migration repair`; Phase 36 + Phase 37 SKIPPED per signature-absent rule (their columns are not in production — consistent with PREREQ-03 framework-only status) ✓ shipped 2026-05-12 (commit `2116c79`)
...
- [⚠] 46-03-andrew-live-uat-execution-PLAN.md — Test-mode UAT partial: 22/28 PASS (18 live + 4 PASS-by-static-evidence) + 5 DEFERRED to live-mode UAT + 1 N/A. Status `human_needed`. Andrew elected to skip remaining Stripe-dependent scenarios for live-mode UAT once live Stripe stack is configured (separate session). Mid-UAT corrections: pushed 33 stale commits (`ac8e263`), fixed Gmail quota cap 400→450 per PROJECT.md spec (`d8267ac`). UAT progress commit `375f147`. 2026-05-12 partial.
- [ ] 46-04-v18-archival-documents-PLAN.md — Append v1.8 delta to `FUTURE_DIRECTIONS.md`; create `.planning/milestones/v1.8-ROADMAP.md`; collapse ROADMAP.md v1.8 section to one-liner. **PAUSED** until live-mode UAT completes.
- [ ] 46-05-v18-tag-and-signoff-PLAN.md — Update STATE.md; commit archival artifacts; create + push annotated `v1.8.0` git tag. **PAUSED** until live-mode UAT completes (tag must be placed on the commit that flips 46-VERIFICATION.md to `status: passed`).
