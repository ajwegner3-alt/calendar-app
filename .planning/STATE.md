...
**Last updated:** 2026-05-11 — Phase 42.5 Plans 03 + 04 (Wave 2 siblings) both complete. Plan 42.5-03 (`refactor(42.5-03) 39054a8`): `app/api/stripe/checkout/route.ts` now accepts `{tier, interval}` body, rejects `tier='branding'` with 400 `{error:'use_consult_link'}` (LD-16), rejects unknown/missing tier with 400 `{error:'unknown_tier'}`, resolves Price ID via `getPriceId(tier, interval)`, adds `tier` to session metadata as debug breadcrumb. Validation order: auth(401) → account(404) → branding(400) → unknown-tier(400) → placeholder(400) → stripe. Plan 42.5-04 (`feat(42.5-04) 1d9aac3`): webhook `handleCheckoutSessionCompleted` derives `plan_tier` via `listLineItems` + `priceIdToTier` reverse-lookup. Phase 41 invariants preserved on both files. Repo typecheck broken callsites reduced from 5 to 4 — all 4 remaining are in `app/(shell)/app/billing/page.tsx` (owned by Plan 42.5-05). Plan 42.5-01 manual `supabase db push --linked` still pending Andrew. Next: Plan 42.5-05 (billing UI 3-card) is the last code plan in Phase 42.5; then Plan 42.5-06 UAT closes SC-4 + SC-5.
...
**Phase:** 42.5 of 46 (+ inserted 42.5/42.6) — in progress (Wave 2 mostly complete)
**Plan:** 42.5-01 + 42.5-02 + 42.5-03 + 42.5-04 complete; 42.5-05 (billing UI) pending; 42.5-06 (UAT) blocked on 42.5-05
**Status:** Wave 2 mostly complete. Plan 42.5-03 (checkout route refactor) shipped as `refactor(42.5-03) 39054a8` — `app/api/stripe/checkout/route.ts` now accepts `{tier, interval}` body, rejects `tier='branding'` with 400 `{error:'use_consult_link'}` (LD-16), rejects unknown/missing tier with 400 `{error:'unknown_tier'}`, resolves Price ID via `getPriceId(tier, interval)`. Validation order is auth(401) → account(404) → branding(400) → unknown-tier(400) → placeholder(400) → stripe; account fetch was moved before body parsing per must_have invariant. `tier` added to session metadata as debug breadcrumb only — webhook (Plan 42.5-04) ignores it and derives plan_tier via `PRICE_ID_TO_TIER` reverse-lookup per LD-14. Phase 41 / LD-18 invariants preserved (auth check, account lookup, customer upsert race guard, NO_STORE headers, success/cancel URLs, return shape). Repo typecheck broken callsites reduced from 5 → 4 — remaining 4 are all in `app/(shell)/app/billing/page.tsx` (owned by Plan 42.5-05). Plan 42.5-01 manual `supabase db push --linked` still owed by Andrew (column does not yet exist on live DB; webhook UPDATE will throw and Stripe will retry until migration lands — no data loss).
**Last activity:** 2026-05-11 — Plan 42.5-03 executed: refactored `app/api/stripe/checkout/route.ts` body parsing + validation; reordered so account fetch precedes tier validation; added `tier` to session metadata; updated docstring response-shapes table. Skipped live curl tests (no session cookie in agent context — deferred to Plan 42.5-06 UAT). SUMMARY at `.planning/phases/42.5-multi-tier-stripe-and-schema/42.5-03-SUMMARY.md`.
